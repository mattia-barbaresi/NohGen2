...
def ncd_similarity(ind1, ind2, tps, classes):
    s1 = mkv.generate_with_weights(
        tps=tps, weights=ind1, n_seq=constants.NUM_SEQS, occ_per_seq=constants.SEQUENCE_LENGTH,
        start_pool=classes["sp"])
    s2 = mkv.generate_with_weights(
        tps=tps, weights=ind2, n_seq=constants.NUM_SEQS, occ_per_seq=constants.SEQUENCE_LENGTH,
        start_pool=classes["sp"])
    return metrics.compute_ncd(''.join(s1), ''.join(s2))


# similar = 0, dissimilar = 1
def ncd_dissimilarity(ind1, ind2, tps, classes):
    return 1 - ncd_similarity(ind1, ind2, tps, classes)
...
def gen_sequences(individual, tps, classes):
    return mkv.generate_with_weights(
        tps=tps, weights=individual, n_seq=constants.NUM_SEQS, occ_per_seq=constants.SEQUENCE_LENGTH, start_pool=classes["sp"])
...
def eval_fitness(individual, tps, classes, patterns):
    ...
    """
    sequences = gen_sequences(individual, tps, classes)
...
# on genotype
def eval_fitness_and_novelty_genotype(individual, tps, classes, patterns, population, archive):
...
def eval_fitness_and_novelty_phenotype_ncd(individual, tps, classes, patterns, population, archive):
    fit = eval_fitness(individual, tps, classes, patterns)
    novelty_search.archive_assessment(individual, fit, archive,
                                      dissim_fun=(lambda x,y: ncd_dissimilarity(x,y,tps, classes)))
    nov = novelty_search.novelty(individual, population, archive,
                                 simil_fun=lambda x,y: ncd_similarity(x,y,tps,classes),
                                 dissimil_fun=(lambda x,y: ncd_dissimilarity(x,y,tps,classes)))
    return fit, nov


# use novelty_search in phenotype = on generated sequences (no population, string similarity)
def eval_fitness_and_novelty_phenotype(individual, tps, classes, patterns, archive):
    sequences = gen_sequences(individual, tps, classes)
    # fitness
    res = fc.evaluate_sequences2(sequences, classes["fc"], patterns)
